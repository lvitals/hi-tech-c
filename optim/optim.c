...
int get_line()
{
    ...
    charsLeft = 0;
    while ((c = getchar()) != '\n' && c != EOF && c != CPMEOF)
        if ((size_t)charsLeft < sizeof(inp_buf) - 1 && c != '\r')
            inp_buf[charsLeft++] = c;
    if (c == CPMEOF)
        while ((c = getchar()) != EOF)
            ;
...
void sub_122f()
{
    ...
    { /* set initial values for symbols */
        if (ilist->tType == T_SYMBOL)
        {
    ...
    for (ilist = switchVectors; ilist; ilist = ilist->pNext)
    { /* do the same for the jump tables */
...
bool sub_24c0()
{
    ...
    else if (gPi->iLhs->tType == T_REG && gPi->iRhs->tType == T_REG)
    { /* 2824 */
...
term_t *evalExpr()
{
    ...
    do
    { /* REDUCE loop */
        for (;; tokType = get_token())
        { /* SHIFT loop */
            if (tokType == T_STRING)
            { /* in expressions "A" is treated as ascii value of A */
            ...
            if (T_FWD <= tokType &&
                tokType <= T_CONST)
            { /* get the term, note two terms together is an error */
                if (expectOp)
                    exp_err();
            ...
            else if (T_UPLUS <= tokType && tokType <= T_LASTOP)
            { /* get the operator */
...
operand_t *evalOperand()
{
    ...
    case T_REG:
        if (expectCond && yylval.i == REG_C)
        {                     /* check if C is used in jp c context */
    ...
    case T_OPAR:
        if ((tokType = get_token()) == T_REG)
        {
            ...
            if ((tokType = get_token()) == T_CPAR)
            { /* simple (reg) so prep for next token */
                tokType = get_token();
                break;
            }
...
bool sub_47e0(int reg, register inst_t const *pi1, inst_t const *pi2)
{
    ...
    if ((size_t)reg >= sizeof(regTestMasks) / sizeof(regTestMasks[0]))
        fprintf(stderr, "%d\n", reg);
...
sym_t *allocItem()
{
    ...
    if (ps)
    { /* if there is an entry release it*/
 ...
 **************************************************************************/
void freeHashtab()
{ /* not realy needed for systems with large memory */
#ifdef CPM
    allocs = (char *)&hashtab;           /* */
