...
/*********************************************************
 * char searchToken OK++ PMO	       Used in: sub_6D1, sub_4192
 *
 * Token search		(Binary search algorithm is used)
 *********************************************************/
char searchToken(register char *target) {
    // prWarning("searchToken: Searching for token '%s'\n", target);
    uint8_t hi, lo, mid;
    char cmp;
    ...
    // Check for the SUB (ASCII 26) character
    if ((unsigned char)*target == 26) {
        prWarning("Skipping control character SUB (ASCII 26) \n");
        return -2;  // Special value to indicate a SUB character was found
    }
...
void parseStmt() {
    ...
    while ((ch = getToken()) != (char *)~0) { /* get_token    */
        ...
        // Check for SUB (ASCII 26) and exit if encountered
        if ((unsigned char)*ch == 26) {
            normalExit();  // Call normalExit to terminate successfully
            return;  // This will never be reached
        }


        tok = searchToken(ch);  // Search token
        if (tok == -2) {
            normalExit();  // Exit successfully if SUB (ASCII 26) is encountered
        }
        ...
        case LOR:  /* "||" */
        default:
            prWarning("Encountered unknown token ID: %d\n", tok);
            badIntCode();
        }
    }
    leaveBlock();
...
int sub_283E(register node_t *sa, int par) {
    ...
    l2z = &codeFrag[(unsigned char)sa->a_c3[par]];
    l7  = 0;
    ...
    for (;;) {
        ...
        case 'L':
            if (p3 > 0) { /* m27: */
                la2  = sa;
                lc10 = p3 - 1;
                lz1  = &codeFrag[(unsigned char)sa->a_c3[(unsigned char)lc10]];
            } else {
                la2  = sa->info.np[0];
                lc10 = la2->a_c1 - 1;
                lz1  = &codeFrag[(unsigned char)la2->a_c3[(unsigned char)lc10]];
            }
            break;
        case 'R':
            la2  = sa->info.np[1];
        ...
        case 'G':
        case 'X':
            lc4 = lc7 == 'G' ? la2->a_reg[(unsigned char)lc10] : la2->a_uc9[(unsigned char)lc10];
...
void sub_3564(register node_t *sa) {
    ...
    for (l1 = 0; l1 != sa->a_c1; l1++) {
        if ((l2z = &codeFrag[sa->a_c3[(unsigned char)l1]])->p_6) {
...
node_t *parseExpr() {
    ...
    for (;;) {
        ...
        l4 = searchToken(l2); /* Token search */ /* m12: */
        if (l4 == -2) {
            normalExit();  // Exit successfully if SUB (ASCII 26) is encountered
        }
...
_Noreturn void normalExit(void) {
    ...
    exit(0);  // Exit with success code 0 (normal termination)
}
...
void *allocMem(size_t size) {
    register char *ptr = NULL; // Initialize ptr to NULL

    do {
        if ((ptr = malloc(size))) { // Assign the result of malloc to ptr
            goto done;
        }
